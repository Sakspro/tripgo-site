...
let fallback = null;
try {
  fallback = require("../data/inspired-cache.json");
} catch (e) {
  fallback = null;
}
...
module.exports = async function handler(req, res) {
  ...
  if (!force && !stale) {
    ...
    res.setHeader("X-Inspired-Cache", "hit");
  ...
  try {
    ...
    res.setHeader("Cache-Control", "public, s-maxage=3600, stale-while-revalidate=7200");
    res.setHeader("X-Inspired-Cache", force ? "refresh" : "miss");
    res.setHeader("X-Inspired-Hash", hash);
    return res.status(200).json(data);
  } catch (err) {
    if (cache.data) {
      res.setHeader("X-Inspired-Cache", "stale-error");
      return res.status(200).json(Object.assign({}, cache.data, { stale: true }));
    }
    if (fallback) {
      res.setHeader("X-Inspired-Cache", "fallback");
      return res.status(200).json(fallback);
    }
    return res.status(502).json({ error: "Could not sync inspired data from Trip.com" });
  }
};
